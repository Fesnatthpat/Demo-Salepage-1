        ...
        <div class="py-8">
            ...
            <div class="mb-8 flex flex-col md:flex-row md:items-end md:justify-between gap-4">
                ...
                <div class="text-right">
                    <span class="text-xs font-medium text-gray-500 uppercase tracking-widest">Total Logs: {{ $activities->total() }}</span>
                </div>
            </div>
            ...
            @if ($filter_admin_name)
                <div class="mb-6 flex items-center justify-between rounded-xl bg-blue-500/10 border border-blue-500/20 p-4 text-blue-200 backdrop-blur-sm">
                    <div class="flex items-center gap-3">
                        ...
                        <p>กำลังแสดงรายการของ: <span class="font-bold text-white text-lg ml-1">{{ $filter_admin_name }}</span></p>
                    </div>
            ...
            <div class="space-y-6">
                @forelse ($activities as $activity)
                    ...
                    <div class="group overflow-hidden rounded-2xl bg-gray-800/50 border border-gray-700 shadow-sm transition-all duration-300 hover:shadow-{{ $color }}-500/10 hover:border-{{ $color }}-500/40">
                        ...
                        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-gray-800/80 px-5 py-4 border-b border-gray-700/50">
                            <div class="flex items-center gap-4">
                                <div class="flex h-12 w-12 items-center justify-center rounded-xl bg-{{ $color }}-500/10 border border-{{ $color }}-500/20 text-{{ $color }}-400 shadow-inner">
                                    <i class="{{ $config['icon'] }} text-xl"></i>
                                </div>

                                <div>
                                    <div class="flex items-center gap-2 mb-0.5">
                                        <span class="text-sm font-bold uppercase tracking-wide text-{{ $color }}-400">
                                            {{ $config['label'] }}
                                        </span>
                                    ...
                                    <div class="text-xs text-gray-400 flex items-center gap-2">
                                        <span class="flex items-center gap-1">
                                            ...
                                            <span class="font-semibold text-blue-400">{{ $activity->admin->name ?? 'Unknown' }}</span>
                                        </span>
                                            ...
                                            @foreach ($activity->changes['new'] as $key => $newValue)
                                                ...
                                                @if ($oldValue != $newValue)
                                                    <div class="grid grid-cols-12 gap-2 px-4 py-3 text-[13px] font-mono hover:bg-white/[0.02] transition-colors items-center">
                                                        <div class="col-span-4 sm:col-span-3 text-gray-400 font-medium truncate" title="{{ $key }}">
                                                            {{ $translatedKey }}
                                                        </div>
                                        ...
                                        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-0 divide-y md:divide-y-0 divide-gray-800 px-4 py-2">
                                            ...
                                            @foreach ($data as $key => $val)
                                                ...
                                                <div class="flex justify-between py-2 border-b border-gray-800/50 last:border-0 text-[12px] font-mono">
                                                    ...
                                                    <span class="text-gray-300 truncate ml-4 max-w-[60%]" title="{{ is_array($val) ? 'Array' : $val }}">
                                                        {{ is_array($val) ? '{...}' : ($val ?: '-') }}
                                                    </span>
                                                </div>
                                            @endforeach
