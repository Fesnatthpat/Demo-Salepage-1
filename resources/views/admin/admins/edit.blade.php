...
@section('title', 'แก้ไขผู้ดูแลระบบ | TIDJAI Admin')
@section('page-title', 'แก้ไขผู้ดูแลระบบ: ' . $admin->name)
...
@section('content')
    <div class="max-w-4xl mx-auto">
        <div class="mb-6 flex justify-between items-center">
            ...
            <span class="text-xs text-gray-500 uppercase tracking-widest">Admin ID: #{{ $admin->id }}</span>
        </div>
        ...
        <div class="bg-gray-800 rounded-2xl shadow-xl border border-gray-700 overflow-hidden">
            <div class="p-6 sm:p-8">
                <form action="{{ route('admin.admins.update', $admin->id) }}" method="POST">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="col-span-2 sm:col-span-1">
                            ...
                            <select name="is_active" id="is_active"
                                ...
                                required>
                                <option value="1" {{ old('is_active', $admin->is_active) ? 'selected' : '' }}>ใช้งานได้ปกติ (Active)</option>
                                <option value="0" {{ !old('is_active', $admin->is_active) ? 'selected' : '' }}>ระงับการใช้งาน (Disabled)</option>
                            </select>
                            @error('is_active')
                                <p class="mt-2 text-sm text-red-400 flex items-center"><i class="fas fa-exclamation-circle mr-1"></i> {{ $message }}</p>
                            @enderror
                        ...
                        <div class="col-span-2 mt-4">
                            <div class="rounded-xl bg-indigo-900/20 p-4 border border-indigo-500/20">
                                <div class="flex">
                                    <div class="flex-shrink-0 text-indigo-400">
                                        <i class="fas fa-info-circle text-lg"></i>
                                    </div>
                                    <div class="ml-3">
                                        <h3 class="text-sm font-bold text-indigo-300 uppercase tracking-wider">การเปลี่ยนรหัสผ่าน</h3>
                                        <p class="mt-1 text-sm text-gray-400">เว้นว่างไว้ทั้งสองช่องหากไม่ต้องการเปลี่ยนรหัสผ่านเดิม</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        {{-- New Password --}}
                        <div class="col-span-2 sm:col-span-1">
                            <label for="password" class="block text-sm font-semibold text-gray-300 mb-2">รหัสผ่านใหม่</label>
                            ...
                            <div class="mt-3 bg-gray-900/50 p-2 rounded-lg border border-gray-700">
                                ...
                                <div class="flex justify-between items-center mt-2">
                                    <p id="strength-text" class="text-[10px] text-gray-500 uppercase tracking-widest font-bold">ความปลอดภัย: <span>ว่าง</span></p>
                        ...
                        <div class="col-span-2 sm:col-span-1">
                            <label for="password_confirmation" class="block text-sm font-semibold text-gray-300 mb-2">ยืนยันรหัสผ่านใหม่</label>
                    ...
                    <div class="mt-10 flex items-center justify-end space-x-4 border-t border-gray-700 pt-8">
                        <a href="{{ route('admin.admins.index') }}" class="px-6 py-3 rounded-lg text-sm font-semibold text-gray-400 hover:text-white transition-colors">ยกเลิก</a>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        passwordInput.addEventListener('input', function() {
            ...
            if (val.length === 0) {
                ...
                // บังคับเงื่อนไขเดียวกับ Backend Validation
            ...
            // อัปเดต UI (ใช้ Inline Style เพื่อชนะ CSS Class)
            ...
            if (strength === 0) {
                ...
                textSpan.textContent = 'ปานกลาง (เพิ่มตัวเล็ก/ใหญ่)';
                textSpan.style.color = '#f97316';
                iconDiv.style.color = '#f97316';
            } else if (strength <= 75) {
                bar.style.backgroundColor = '#eab308'; // yellow-500
                textSpan.textContent = 'ดี (เพิ่มสัญลักษณ์)';
                textSpan.style.color = '#eab308';
                iconDiv.style.color = '#eab308';
            } else {
