...
@section('content')
    <div class="container mx-auto pb-24 max-w-4xl">
        ...
        <form action="{{ route('admin.birthday-promotion.update', $birthdayPromotion->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-12">
                        ...
                        <div class="space-y-8">
                            ...
                            <div class="space-y-4">
                                ...
                                <div class="relative group">
                                    <div class="w-full aspect-[20/13] bg-gray-900 rounded-2xl border-2 border-dashed border-gray-700 overflow-hidden relative transition-all group-hover:border-pink-500/50">
                                        @php
                                            $imageUrl = $birthdayPromotion->image_path ? asset('storage/' . $birthdayPromotion->image_path) : 'https://placehold.co/1200x780/111827/4b5563?text=No+Image';
                                        @endphp
                                        <img src="{{ $imageUrl }}" class="w-full h-full object-cover" id="birthday-image-preview">
                            ...
                            <div class="space-y-2">
                                ...
                                <input type="text" name="title" value="{{ old('title', $birthdayPromotion->title) }}" required
                            ...
                            <div class="space-y-2">
                                ...
                                <textarea name="message" rows="4" required
                                    ...
                                    oninput="document.getElementById('preview-message').innerText = this.value">{{ old('message', $birthdayPromotion->message) }}</textarea>
                            </div>
                        </div>
                            ...
                            <div class="w-full max-w-[280px] bg-[#ebebeb] rounded-[2.5rem] p-4 shadow-2xl border-[6px] border-gray-900">
                                <div class="bg-white rounded-2xl overflow-hidden shadow-md flex flex-col">
                                    <div class="aspect-[20/13] w-full bg-gray-200">
                                        <img id="preview-img-card" src="{{ $imageUrl }}" class="w-full h-full object-cover">
                                    </div>
                                    <div class="p-4 space-y-1">
                                        <div id="preview-title" class="text-pink-500 font-bold text-[10px] uppercase">{{ $birthdayPromotion->title }}</div>
                                        <div class="text-black font-bold text-base">คุณ ชื่อลูกค้า</div>
                                        <div id="preview-message" class="text-gray-600 text-xs leading-relaxed">{{ $birthdayPromotion->message }}</div>
                                    </div>
                                    <div class="px-4 pb-4">
                        ...
                        <div class="space-y-2">
                            ...
                            <select name="promotion_id" class="w-full bg-gray-900 border border-gray-700 rounded-xl px-4 py-3 text-gray-100 focus:ring-2 focus:ring-pink-500/50 outline-none">
                                ...
                                @foreach ($promotions as $promo)
                                    <option value="{{ $promo->id }}" {{ old('promotion_id', $birthdayPromotion->promotion_id) == $promo->id ? 'selected' : '' }}>
