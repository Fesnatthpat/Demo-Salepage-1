    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        ...
        <div class="flex flex-wrap items-center gap-2 bg-white p-1.5 rounded-xl shadow-sm border border-gray-100">
            <a href="{{ route('admin.dashboard', ['period' => 'today']) }}"
                class="filter-pill px-4 py-2 text-sm font-medium rounded-lg {{ $period == 'today' ? 'active text-white' : 'text-gray-600 hover:bg-gray-50' }}">
                วันนี้
            </a>
            <a href="{{ route('admin.dashboard', ['period' => 'last_7_days']) }}"
                class="filter-pill px-4 py-2 text-sm font-medium rounded-lg {{ $period == 'last_7_days' ? 'active text-white' : 'text-gray-600 hover:bg-gray-50' }}">
                7 วัน
            </a>
            <a href="{{ route('admin.dashboard', ['period' => 'this_month']) }}"
                class="filter-pill px-4 py-2 text-sm font-medium rounded-lg {{ $period == 'this_month' ? 'active text-white' : 'text-gray-600 hover:bg-gray-50' }}">
                เดือนนี้
            </a>
            <a href="{{ route('admin.dashboard', ['period' => 'last_30_days']) }}"
                class="filter-pill px-4 py-2 text-sm font-medium rounded-lg {{ $period == 'last_30_days' ? 'active text-white' : 'text-gray-600 hover:bg-gray-50' }}">
                30 วัน
            </a>
        </div>
    </div>
        ...
        <div class="stat-card bg-white p-6 rounded-2xl shadow-sm border border-gray-100 relative overflow-hidden">
            <div class="flex justify-between items-start">
                <div>
                    ...
                    <h3 class="text-3xl font-bold text-gray-800">฿{{ number_format($totalSales, 0) }}</h3>
                </div>
        ...
        <div class="stat-card bg-white p-6 rounded-2xl shadow-sm border border-gray-100 relative overflow-hidden">
            <div class="flex justify-between items-start">
                <div>
                    ...
                    <h3 class="text-3xl font-bold text-gray-800">฿{{ number_format($avgOrderValue, 0) }}</h3>
                </div>
        ...
        <div class="stat-card bg-white p-6 rounded-2xl shadow-sm border border-gray-100 relative overflow-hidden">
            <div class="flex justify-between items-start">
                <div>
                    ...
                    <h3 class="text-3xl font-bold text-gray-800">{{ number_format($newCustomers) }}</h3>
                </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
        ...
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
            ...
            <div class="mt-6 space-y-2">
                {{-- Legend จะถูกสร้างด้วย JS หรือทำ Custom Legend ตรงนี้ก็ได้ --}}
            </div>
        </div>
    </div>
                            ...
                            <tr class="hover:bg-gray-50 transition-colors border-b border-gray-50 last:border-0">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <span class="text-gray-700">{{ $order->user->name ?? 'Guest' }}</span>
                                    </div>
                                </td>
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex justify-between mb-1">
                                <span
                                    class="text-sm font-medium text-gray-700 truncate pr-2">{{ $product->pd_sp_name ?? 'Product Deleted' }}</span>
                                <span class="text-sm font-bold text-gray-900">{{ number_format($item->total_quantity) }}
                                    ชิ้น</span>
                            </div>
                            <div class="w-full bg-gray-100 rounded-full h-1.5">
                                <div class="bg-indigo-500 h-1.5 rounded-full" style="width: {{ $item->percent }}%"></div>
                            </div>
                        </div>
                    </div>
                @empty
