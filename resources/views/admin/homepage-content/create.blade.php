...
@section('title', 'เพิ่มเนื้อหาหน้าหลักใหม่')
@section('page-title', 'เพิ่มเนื้อหาหน้าหลักใหม่')

@section('content')
    <div class="container mx-auto p-4">
        <h1 class="text-2xl font-semibold text-gray-800 mb-6">เพิ่มเนื้อหาหน้าหลักใหม่</h1>

        <div class="bg-white shadow rounded-lg p-6">
            <form action="{{ route('admin.homepage-content.store') }}" method="POST">
                @csrf
                ...
                <div class="mb-4">
                    ...
                    <input type="text"
                        ...
                        id="item_key" name="item_key" value="{{ old('item_key') }}">
                    @error('item_key')
                        <p class="text-red-500 text-xs italic">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <select
                        ...
                        <option value="text" {{ old('type') == 'text' ? 'selected' : '' }}>ข้อความ</option>
                        <option value="image" {{ old('type') == 'image' ? 'selected' : '' }}>รูปภาพ</option>
                        <option value="icon" {{ old('type') == 'icon' ? 'selected' : '' }}>ไอคอน (SVG)</option>
                        <option value="link" {{ old('type') == 'link' ? 'selected' : '' }}>ลิงก์</option>
                        <option value="collection" {{ old('type') == 'collection' ? 'selected' : '' }}>คอลเลกชัน (JSON)
                        </option>
                    </select>
                    @error('type')
                        <p class="text-red-500 text-xs italic">{{ $message }}</p>
                ...
                <div class="mb-4">
                    ...
                    <p class="text-gray-600 text-xs italic mt-1">ใช้สำหรับประเภทข้อความ, รูปภาพ (path), ไอคอน (SVG Code) หรือลิงก์</p>
                    @error('value')
                        <p class="text-red-500 text-xs italic">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <textarea
                        ...
                        id="data" name="data" rows="5">{{ old('data') }}</textarea>
                    <p class="text-gray-600 text-xs italic mt-1">ใช้สำหรับประเภทคอลเลกชัน (เช่น {"title": "...", "description": "..."})</p>
                    @error('data')
                        <p class="text-red-500 text-xs italic">{{ $message }}</p>
                    @enderror
                </div>
