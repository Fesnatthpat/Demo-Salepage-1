            ...
            <div class="card bg-white shadow-sm border border-gray-200">
                <div class="card-body p-6 gap-5">
                    ...
                    <div class="form-control w-full">
                        ...
                        <input type="text" name="name" class="input input-bordered w-full" value="{{ old('name', $promotion->name ?? '') }}" required />
                    </div>

                    {{-- ✅ เพิ่มส่วนเลือก Condition Type --}}
                    <div class="form-control w-full">
                        <label class="label"><span class="label-text font-semibold">รูปแบบเงื่อนไขการซื้อ</span></label>
                        <div class="flex flex-col gap-2">
                            <label class="flex items-center gap-3 p-3 border rounded-lg cursor-pointer hover:bg-gray-50 transition-colors {{ old('condition_type', $promotion->condition_type ?? 'any') == 'any' ? 'border-primary bg-primary/5' : '' }}">
                                <input type="radio" name="condition_type" value="any" class="radio radio-primary radio-sm" {{ old('condition_type', $promotion->condition_type ?? 'any') == 'any' ? 'checked' : '' }} />
                                <div>
                            ...
                            <label class="flex items-center gap-3 p-3 border rounded-lg cursor-pointer hover:bg-gray-50 transition-colors {{ old('condition_type', $promotion->condition_type ?? 'any') == 'all' ? 'border-primary bg-primary/5' : '' }}">
                                <input type="radio" name="condition_type" value="all" class="radio radio-primary radio-sm" {{ old('condition_type', $promotion->condition_type ?? 'any') == 'all' ? 'checked' : '' }} />
                                <div>
                    ...
                    <div class="form-control w-full">
                        ...
                        <textarea name="description" class="textarea textarea-bordered h-24 resize-none">{{ old('description', $promotion->description ?? '') }}</textarea>
                    </div>
                                ...
                                <div class="card-body p-4">
                                    ...
                                    <div class="form-control w-full">
                                        ...
                                        <select id="giftable-products-select" name="giftable_product_ids[]" multiple placeholder="ค้นหาของแถม..." autocomplete="off">
                                            ...
                                            @endphp
                                            @foreach ($products as $p)
                                                <option value="{{ $p->pd_sp_id }}" {{ $selectedGiftIds->contains((string) $p->pd_sp_id) ? 'selected' : '' }}>
                                                    {{ $p->pd_sp_name }}
                                                </option>
                                            @endforeach
                                        </select>
