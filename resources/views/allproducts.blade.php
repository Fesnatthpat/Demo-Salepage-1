...
@section('content')
    ...
    <style>
        ...
        .product-title-fixed {
            ...
            min-height: 2.5rem;
            /* ความสูงเผื่อ 2 บรรทัดสำหรับฟอนต์เล็ก */
            line-height: 1.25;
        }
        ...
        <div class="container mx-auto px-3 sm:px-4 md:px-6 max-w-7xl">
            ...
            <div class="flex flex-col gap-5 md:gap-8">
                ...
                <main class="w-full">
                    ...
                    <div
                        class="w-full pb-4 sm:pb-6 pt-2 sm:pt-4 bg-white rounded-xl sm:rounded-2xl shadow-sm border border-gray-100">
                        <div class="px-2 sm:px-4">
                            <div
                                class="relative w-full aspect-[16/10] md:aspect-[2.5/1] lg:aspect-[3/1] bg-gray-50 group rounded-xl sm:rounded-2xl overflow-hidden shadow-inner border border-gray-100">
                    ...
                    <div
                        class="w-full py-3 sm:py-4 rounded-xl mt-4 mb-5 shadow-sm bg-red-600 overflow-hidden relative group select-none">
                                            ...
                                            <div class="swiper-slide !h-auto">
                                                <a href="{{ route('allproducts', array_merge(request()->except('page'), ['category' => $menuValue])) }}"
                                                    ...
                                                    <div
                                                        ...
                                                        @else
                                                            <i
                                                                class="{{ $menu->icon ?? 'fas fa-th-large' }} text-red-600 text-lg sm:text-xl md:text-2xl"></i>
                                                        @endif
                                                    </div>
                                                    <span
                    ...
                    {{-- ★★★ SORTING BAR ★★★ --}}
                    <div
                        class="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white p-4 sm:p-4 rounded-xl sm:rounded-2xl shadow-sm border border-gray-100 mb-5 gap-3 sm:gap-4">
                        <div>
                            <h2
                                class="text-gray-800 font-extrabold text-base sm:text-lg md:text-xl flex items-center gap-2">
                                สินค้าทั้งหมด
                                <span
                                    class="bg-gray-100 text-gray-500 px-2 py-0.5 rounded-md text-[10px] sm:text-xs font-bold border border-gray-200">
                        ...
                        <form id="sortForm" action="{{ route('allproducts') }}" method="GET" class="w-full sm:w-auto">
                            ...
                            <div class="flex items-center gap-2 w-full">
                                <label class="text-xs sm:text-sm font-bold text-gray-500 whitespace-nowrap shrink-0"><i
                                        class="fas fa-sort-amount-down-alt mr-1"></i> เรียงตาม:</label>
                                <select name="sort" onchange="document.getElementById('sortForm').submit();"
                                    ...
                                    <option value="bestseller" {{ request('sort') == 'bestseller' ? 'selected' : '' }}>
                                        ขายดี</option>
                                    <option value="price_asc" {{ request('sort') == 'price_asc' ? 'selected' : '' }}>ราคา:
                                        ต่ำ - สูง</option>
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 sm:gap-4 md:gap-6">
                            @foreach ($products as $product)
                                @if ($product)
                                    ...
                                    <div
                                        class="relative bg-white border border-gray-100 shadow-sm hover:shadow-lg hover:border-red-100 transition-all duration-300 rounded-xl sm:rounded-2xl overflow-hidden flex flex-col h-full group">

                                        <a href="{{ route('product.show', $product->pd_sp_id) }}"
                                            class="block relative aspect-square overflow-hidden bg-gray-50/50">
                                            @if (($product->pd_sp_stock ?? 0) <= 0)
                                                <div
                                                    class="absolute inset-0 flex items-center justify-center z-10 bg-white/60 backdrop-blur-[2px]">
                                                    <span
                                                        class="bg-gray-800 text-white text-[10px] sm:text-xs px-3 sm:px-4 py-1 sm:py-1.5 rounded-full font-bold uppercase tracking-wider shadow-md">สินค้าหมด</span>
                                                </div>
                                            @endif
                                            ...
                                            <div class="absolute top-2 left-2 flex flex-col gap-1.5 items-start z-10">
                                                @if ($isOnSale)
                                                    <div
                                                        class="bg-red-600 text-white px-1.5 py-0.5 sm:px-2 sm:py-1 rounded text-[9px] sm:text-[10px] font-black shadow-sm uppercase tracking-wide border border-red-500">
                                                        ลด ฿{{ number_format($discountAmount, 2) }}
                                                    </div>
                                                @endif
                                                @if ($product->gifts_per_item)
                                                    <div
                                                        class="bg-pink-500 text-white px-1.5 py-0.5 sm:px-2 sm:py-1 rounded text-[9px] sm:text-[10px] font-black shadow-sm flex items-center gap-1 uppercase tracking-wide border border-pink-400">
                                                        <i class="fas fa-gift text-[8px] sm:text-[9px]"></i> แถม
                                                        {{ $product->gifts_per_item }}
                                                    </div>
                                                @endif
                                            </div>
                                        </a>
                                        ...
                                        <div class="p-3 sm:p-4 flex flex-col flex-1">
                                            <h2
                                                class="text-xs sm:text-sm font-bold text-gray-800 leading-tight product-title-fixed mb-1 sm:mb-2 group-hover:text-red-600 transition-colors">
                                                <a href="{{ route('product.show', $product->pd_sp_id) }}">
                                            ...
                                            <div class="flex items-center justify-between mb-2">
                                                <p
                                                    class="text-[9px] sm:text-[10px] font-bold {{ ($product->pd_sp_stock ?? 0) > 0 ? 'text-emerald-500' : 'text-red-500' }} flex items-center gap-1">
                                                    <span class="relative flex h-1.5 w-1.5 sm:h-2 sm:w-2">
                                                        <span
                                                            class="animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 {{ ($product->pd_sp_stock ?? 0) > 0 ? 'bg-emerald-400' : 'hidden' }}"></span>
                                                        <span
                                                            class="relative inline-flex rounded-full h-1.5 w-1.5 sm:h-2 sm:w-2 {{ ($product->pd_sp_stock ?? 0) > 0 ? 'bg-emerald-500' : 'bg-red-500' }}"></span>
                                                    </span>
                    ...
                    @else
                        <div
                            class="flex flex-col items-center justify-center py-16 sm:py-24 bg-white rounded-2xl border-2 border-dashed border-gray-200 text-center">
                            <div class="w-20 h-20 bg-gray-50 rounded-full flex items-center justify-center mb-4">
                                <i class="fas fa-box-open text-4xl text-gray-300"></i>
                            </div>
                            <h3 class="text-lg sm:text-xl font-bold text-gray-700 mb-2">ไม่พบสินค้าที่คุณค้นหา</h3>
                            <p class="text-xs sm:text-sm text-gray-400 mb-6">ลองเปลี่ยนคำค้นหา
                                หรือเลือกดูหมวดหมู่สินค้าอื่นๆ</p>
                            <a href="{{ route('allproducts') }}"
                                class="px-6 py-2.5 bg-red-50 text-red-600 hover:bg-red-600 hover:text-white font-bold rounded-xl transition-colors border border-red-100 hover:border-red-600 shadow-sm">
