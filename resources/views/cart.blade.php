                                ...
                                <div class="flex flex-row justify-between items-center md:flex-col md:items-end gap-4 w-full md:w-auto mt-2 md:mt-0 pl-9 md:pl-0">
                                    ...
                                    <div class="flex flex-col sm:flex-row items-end sm:items-center gap-3">
                                        ...
                                        <button type="button" class="cart-action-btn text-red-500 hover:text-red-700 font-medium text-sm md:text-base underline md:no-underline md:btn md:btn-ghost md:btn-sm md:text-red-500"
                                            data-url="{{ route('cart.remove', $item->id) }}" data-method="DELETE">ลบรายการ</button>
                                    </div>
                                </div>
                            </div>
                        @endforeach
                        ...
                        <div class="flex flex-col lg:flex-row justify-end gap-5 mt-10">
                            <div class="w-full lg:w-[400px]">
                                ...
                                <div class="flex justify-between mt-2 text-base text-red-500">
                                    <div>ส่วนลดรวม</div>
                                    <div class="font-medium">-฿<span id="discount-display">{{ number_format($summaryTotalOriginal - $summaryTotalPrice) }}</span></div>
                                </div>
                                ...
                                <div class="flex justify-between items-center mb-6">
                                    ...
                                    <div>
                                        <h1 class="text-emerald-600 font-bold text-2xl md:text-3xl">฿<span id="total-display">{{ number_format($summaryTotalPrice) }}</span></h1>
                                    </div>
                                </div>
                                <div class="flex flex-col gap-3">
                                    @auth
                                        <button type="submit" id="checkout-btn" class="btn btn-success text-white w-full text-lg h-12">ดำเนินการชำระเงิน</button>
                                    @endauth
                                    @guest
                                        <a href="{{ route('login') }}" class="btn btn-success text-white w-full text-lg h-12 flex items-center justify-center">เข้าสู่ระบบเพื่อชำระเงิน</a>
                                    @endguest
                                    <a href="{{ route('allproducts') }}" class="btn btn-outline border-gray-300 text-gray-600 hover:bg-gray-50 w-full flex items-center justify-center">ซื้อสินค้าต่อ</a>
                                </div>
                            </div>
                        </div>
                    @else
