        ...
        @forelse ($contacts as $contact)
            <div class="bg-white rounded-3xl shadow-xl overflow-hidden flex flex-col lg:flex-row transition-transform duration-300 hover:-translate-y-1 hover:shadow-2xl">
                ...
                <div class="p-8 lg:p-12 lg:w-1/2 flex flex-col justify-center">
                    <h2 class="text-3xl font-bold text-gray-800 mb-4">{{ $contact->title }}</h2>
                    
                    @if($contact->content)
                        <p class="text-gray-600 mb-8 leading-relaxed">{!! nl2br(e($contact->content)) !!}</p>
                    @endif
                        ...
                        @if($contact->address)
                            <div class="flex items-start group">
                                ...
                                <div class="ml-5 mt-1">
                                    ...
                                    <span class="text-gray-600 leading-relaxed">{!! nl2br(e($contact->address)) !!}</span>
                                </div>
                            </div>
                        @endif
                        ...
                        @if($contact->phone)
                            <div class="flex items-start group">
                                ...
                                <div class="ml-5 mt-1">
                                    ...
                                    <a href="tel:{{ preg_replace('/[^0-9+]/', '', $contact->phone) }}" class="text-gray-600 hover:text-green-600 transition-colors block">{{ $contact->phone }}</a>
                                </div>
                            </div>
                        @endif
                        ...
                        @if($contact->email)
                            <div class="flex items-start group">
                                ...
                                <div class="ml-5 mt-1">
                                    ...
                                    <a href="mailto:{{ $contact->email }}" class="text-gray-600 hover:text-red-600 transition-colors block">{{ $contact->email }}</a>
                                </div>
                            </div>
                        @endif
                    </div>
