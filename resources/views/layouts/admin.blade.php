        ...
        <aside class="flex-shrink-0 w-64 flex flex-col border-r border-gray-700 transition-all duration-300 bg-gray-800"
            ...
            <nav class="flex-1 px-4 py-4 space-y-2 overflow-y-auto">
                ...
                <a href="{{ route('admin.promotions.index') }}"
                    class="flex items-center px-4 py-3 rounded-lg transition-colors {{ request()->routeIs('admin.promotions.*') ? 'bg-gray-700 text-emerald-400 font-semibold border-l-4 border-emerald-500' : 'text-gray-400 hover:bg-gray-700 hover:text-white' }}">
                    <i class="fas fa-tags mr-3 w-5 text-center"></i>
                    โปรโมชั่น
                </a>
                {{-- Dropdown for Content Management --}}
                <div x-data="{ open: {{ request()->routeIs('admin.faqs.*') || request()->routeIs('admin.favorites.*') || request()->routeIs('admin.contacts.*') ? 'true' : 'false' }} }" class="space-y-1">
                ...
                @if (auth()->guard('admin')->check() && auth()->guard('admin')->user()->role === 'superadmin')
                    ...
                    <a href="{{ route('admin.activity-log.index') }}"
                        class="flex items-center px-4 py-3 rounded-lg transition-colors {{ request()->routeIs('admin.activity-log.*') ? 'bg-gray-700 text-emerald-400 font-semibold border-l-4 border-emerald-500' : 'text-gray-400 hover:bg-gray-700 hover:text-white' }}">
                        <i class="fas fa-history mr-3 w-5 text-center"></i>
                        ประวัติกิจกรรม
                    </a>
                    <a href="{{ route('admin.admins.index') }}"
                        class="flex items-center px-4 py-3 rounded-lg transition-colors {{ request()->routeIs('admin.admins.*') ? 'bg-gray-700 text-emerald-400 font-semibold border-l-4 border-emerald-500' : 'text-gray-400 hover:bg-gray-700 hover:text-white' }}">
