...
@section('content')
        ...
        if (isset($promotions) && $promotions->isNotEmpty()) {
            foreach ($promotions as $promo) {
                ...
                foreach ($promo->actions as $action) {
                    ...
                    if ($action->giftableProducts) $gifts = $gifts->merge($action->giftableProducts);
                }
                $promotionsData[] = [
                    'id' => $promo->id,
                    'logic' => $promo->frontend_logic,
                    'gifts_per_item' => $promo->actions->sum(fn($a) => (int) ($a->actions['quantity_to_get'] ?? 0)),
                    'gifts' => $gifts->unique('pd_sp_id')->map(function ($g) {
                        return [
                            'id' => $g->pd_sp_id,
                            'name' => $g->pd_sp_name,
                    ...
                    'partner_products' => ($promo->partner_products ?? collect())->map(function ($p) {
                        return [
                            'id' => $p->pd_sp_id,
                            'name' => $p->pd_sp_name,
    ...
    @keydown.arrow-left.window="if(isModalOpen) prevImage()" 
    @keydown.arrow-right.window="if(isModalOpen) nextImage()">
                ...
                <div class="lg:col-span-5 p-8 bg-gray-50/50">
                    <div class="sticky top-8">
                        ...
                        <div class="relative aspect-square rounded-2xl bg-white overflow-hidden shadow-sm border border-gray-100 cursor-zoom-in group"
                            ...
                            <img :src="activeImage" class="w-full h-full object-contain p-4 transition-all duration-300 group-hover:scale-105"
                                onerror="this.src='https://via.placeholder.com/600?text=Image+Error'">
                            
                            {{-- Icon แว่นขยาย --}}
                            <div class="absolute bottom-4 right-4 bg-black/10 p-2 rounded-full opacity-0 group-hover:opacity-100 transition-opacity">
                            ...
                            <div class="grid grid-cols-5 gap-3 mt-6">
                                <template x-for="img in images" :key="img">
                                    <button @click="activeImage = img"
                                        ...
                                        :class="activeImage === img ? 'border-red-500 shadow-md transform scale-105' : 'border-transparent opacity-60 hover:opacity-100'">
                ...
                <div class="lg:col-span-7 p-8 lg:p-12 flex flex-col">
                    <div class="flex-1">
                        ...
                        @if ($product->pd_details)
                            <div><h2 class="text-lg font-bold text-gray-800 mb-4">รายละเอียดสินค้า:</h2></div>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <template x-for="option in options" :key="option.id">
                                        <label class="flex items-center gap-4 p-4 rounded-xl border-2 transition-all duration-300 cursor-pointer"
                                            ...
                                            <input type="radio" name="product_option" x-model="selectedOption" :value="option.id"
                            ...
                            <div id="promotion-section" class="mb-10 scroll-mt-24">
                                <div class="p-6 rounded-2xl border-2 border-dashed bg-red-50/30 mb-4 transition-all duration-500"
                                    ...
                                    <h3 class="text-sm font-bold text-gray-800 mb-2 flex items-center gap-2">
                                        ...
                                        <span x-show="isConditionMet" class="badge badge-error badge-sm text-white">ปลดล็อคแล้ว!</span>
                                    </h3>

                                    <template x-if="isConditionMet">
                                        <div class="mb-4 text-sm text-gray-700">
                                            ✅ เงื่อนไขครบถ้วน! คุณได้รับสิทธิ์เลือกฟรี <strong class="text-red-600 text-lg" x-text="giftLimit"></strong> ชิ้น
                                        </div>
                                    </template>
                                        ...
                                        <template x-for="gift in activePromotion.gifts" :key="gift.id">
                                            <label class="flex items-center gap-4 p-3 rounded-xl border-2 transition-all duration-300"
                                                :class="{
                                                    ...
                                                    'bg-white cursor-pointer border-red-200 ring-2 ring-red-100 hover:border-red-300': isConditionMet && !isGiftDisabled(gift.id),
                                                    'border-red-500 bg-red-50 ring-0': selectedGifts.includes(gift.id)
                                                }">
                                                <input type="checkbox" :disabled="!isConditionMet || (selectedGifts.length >= giftLimit && !selectedGifts.includes(gift.id))"
                                                ...
                                                <div class="flex-1 overflow-hidden">
                                                    ...
                                                    <span class="text-xs font-bold" :class="!isConditionMet ? 'text-gray-400' : 'text-red-500'" x-text="!isConditionMet ? 'ล็อค' : 'เลือกฟรี'"></span>
                                                </div>
                                            </label>
                                        </template>
                                    </div>
                    ...
                    <div class="pt-8 border-t border-gray-100 flex flex-col sm:flex-row items-center gap-6">
                        <div class="flex items-center bg-gray-100 rounded-2xl p-1.5 shadow-inner">
                            <button @click="quantity > 1 ? quantity-- : null" class="w-10 h-10 flex items-center justify-center text-gray-500 hover:text-red-600 font-bold text-lg">-</button>
                            <input type="number" x-model.number="quantity" class="w-12 text-center bg-transparent border-none font-black text-gray-900 focus:ring-0 text-lg" readonly>
                            <button @click="quantity++" class="w-10 h-10 flex items-center justify-center text-gray-500 hover:text-red-600 font-bold text-lg">+</button>
                        </div>
                        <div class="flex-1 w-full grid grid-cols-2 gap-4">
                            <button @click="handleAddToCartClick(false)" class="h-14 rounded-2xl border-2 border-red-600 text-red-600 font-bold hover:bg-red-50 transition-all flex items-center justify-center gap-2 text-lg">
                                <span x-show="!isLoading">ใส่ตะกร้า</span>
                                <span x-show="isLoading" class="loading loading-spinner"></span>
                            </button>
                            <button @click="handleAddToCartClick(true)" class="h-14 rounded-2xl bg-red-600 text-white font-bold hover:bg-red-700 shadow-lg shadow-red-500/30 transition-all text-lg">Buy Now</button>
                        </div>
                    </div>
                </div>
            </div>
        ...
        {{-- ========================================== --}}
        {{-- 🖼️ MODAL POPUP & SLIDER (แก้ไขปุ่มตรงนี้) --}}
        {{-- ========================================== --}}
        <div x-show="isModalOpen" 
            ...
            <div class="relative w-full max-w-5xl h-full flex items-center justify-center">
                ...
                {{-- ★ แก้ไข: เปลี่ยน bg-white/10 เป็น bg-black/50 เพื่อให้ตัดกับรูปภาพชัดเจน --}}
                ...
                {{-- Right Button (Next) --}}
                {{-- ★ แก้ไข: ใช้สไตล์เดียวกับปุ่มซ้าย --}}
                <button x-show="images.length > 1" 
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('productPage', (config) => ({
                ...
                get activePromotion() { return this.promotions.length > 0 ? this.promotions[0] : null; },
                get isConditionMet() {
                ...
                get giftLimit() {
                    return (!this.activePromotion || !this.isConditionMet) ? 0 : this.activePromotion.gifts_per_item;
                },
                ...
                init() {
                    ...
                    if (localStorage.getItem('justAddedPartner') === 'true') {
                        ...
                        setTimeout(() => {
                            ...
                            if (promoSection) promoSection.scrollIntoView({ behavior: 'smooth', block: 'center' });
                            Swal.fire({
                                icon: 'success', title: 'ปลดล็อคของแถมแล้ว!', text: 'คุณสามารถเลือกของแถมได้เลย',
                ...
                validateSelection() {
                    ...
                    else if (this.selectedGifts.length > this.giftLimit) this.selectedGifts.splice(this.giftLimit);
                },
                toggleGift(id) {
                    ...
                    else Swal.fire({ icon: 'warning', title: 'เลือกของแถมครบแล้ว', confirmButtonColor: '#ef4444' });
                },
                async addToCartPartner(partnerId) {
                    ...
                    try {
                        const response = await fetch(config.bundleAddUrl, {
                            ...
                            body: JSON.stringify({ main_product_id: parseInt(config.currentProductId), secondary_product_id: parseInt(partnerId), gift_ids: this.selectedGifts })
                        });
                        const data = await response.json();
                        if (data.success) { localStorage.setItem('justAddedPartner', 'true'); window.location.reload(); } 
                        else throw new Error(data.message);
                    } catch (e) { Swal.fire('ข้อผิดพลาด', e.message, 'error'); this.isLoading = false; }
                },
                async handleAddToCartClick(isBuyNow) {
                    if (this.options.length > 0 && !this.selectedOption) { Swal.fire('กรุณาเลือกตัวเลือก', 'เลือกตัวเลือกก่อน', 'warning'); return; }
                    if (this.isConditionMet && this.giftLimit > 0 && this.selectedGifts.length !== this.giftLimit) { Swal.fire({ icon: 'warning', title: 'กรุณาเลือกของแถม', confirmButtonColor: '#ef4444' }); return; }
                    if (this.isLoading) return;
                    this.isLoading = true;
                    try {
                        const response = await fetch(config.standardAction, {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content },
                            body: JSON.stringify({ quantity: parseInt(this.quantity), selected_gift_ids: this.selectedGifts, selected_option_id: this.selectedOption })
                        });
                        const data = await response.json();
                        if (data.success) { 
                    ...
                    } catch (e) { Swal.fire('ข้อผิดพลาด', e.message, 'error'); } finally { this.isLoading = false; }
                }
            }));
        });
    </script>
