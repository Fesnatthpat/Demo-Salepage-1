                            ...
                            <div class="mb-10 p-6 rounded-2xl border-2 border-dashed border-red-100 bg-red-50/30">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                    @foreach ($giftableProducts as $gift)
                                        @php $isLocked = !$gift->is_unlocked; @endphp
                                        <label class="flex items-center gap-4 p-3 rounded-xl border-2 transition-all duration-300
                                            {{ $isLocked ? 'bg-gray-100 border-gray-200 opacity-60 cursor-not-allowed grayscale' : 'bg-white cursor-pointer border-gray-100 hover:border-gray-300' }}"
                                            :class="selectedGifts.includes({{ $gift->pd_sp_id }}) ? 'border-emerald-500 bg-emerald-50' : ''">
                                            ...
                                            <div class="flex-1 overflow-hidden">
                                                <p class="text-xs font-bold text-gray-800 truncate">{{ $gift->pd_sp_name }}</p>
                                                @if($isLocked)
