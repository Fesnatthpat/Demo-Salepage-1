...
@section('content')
    <div class="container mx-auto p-4 min-h-screen flex items-center justify-center bg-gray-50">
        ...
        <div class="w-full max-w-md bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100">
            ...
            <div class="p-6">
                {{-- Countdown Timer --}}
                <div id="timer-container" class="flex items-center justify-center gap-2 mb-6 text-gray-500 text-sm bg-red-50 py-2 rounded-full border border-red-100 transition-all duration-300">
                ...
                <div id="expired-message" class="hidden text-center mb-6 bg-gray-100 py-3 rounded-full text-gray-500 text-sm">
                ...
                <div class="text-center animate-fade-in relative">
                    ...
                    <div class="bg-white p-4 rounded-xl border-2 border-dashed border-gray-300 inline-block mb-4 relative group transition-all duration-500" id="qr-container">
                        ...
                        <div id="qr-overlay" class="absolute inset-0 bg-white/90 backdrop-blur-sm z-10 flex flex-col items-center justify-center opacity-0 pointer-events-none transition-opacity duration-300">
                    ...
                    <div id="save-btn-container" class="flex justify-center gap-3 mb-6 transition-all duration-300">
                        <button onclick="saveQRCode()" class="btn btn-sm btn-outline gap-2 text-gray-600 border-gray-300 hover:bg-gray-50 hover:text-gray-800 hover:border-gray-400 font-normal">
                ...
                {{-- ปุ่มแจ้งชำระเงิน --}}
                <button id="upload-slip-btn" onclick="showUploadSlipPopup()" class="btn w-full bg-[#00B900] hover:bg-[#009900] text-white border-none text-lg h-12 shadow-md shadow-emerald-200 mb-3 transition-all duration-300">
                    แจ้งชำระเงิน / แนบสลิป
                </button>

                <a href="{{ route('order.history') }}" class="btn btn-ghost btn-sm w-full text-gray-400 font-normal hover:bg-transparent hover:text-gray-600">
    ...
    <form id="slip-upload-form" action="{{ route('payment.slip.upload', ['orderCode' => $order->ord_code]) }}" method="POST" enctype="multipart/form-data" class="hidden">
        @csrf
