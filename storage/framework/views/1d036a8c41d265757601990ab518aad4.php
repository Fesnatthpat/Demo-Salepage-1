    ...
    <div class="container mx-auto pb-24 max-w-5xl" x-data="siteSettings()">
        ...
        <div class="flex items-center gap-2 mb-8 bg-gray-800/50 p-1.5 rounded-2xl border border-gray-700 w-fit mx-auto md:mx-0 shadow-lg backdrop-blur-md">
            ...
            <button type="button" 
                ...
                class="px-6 py-2.5 rounded-xl font-bold transition-all flex items-center gap-2 text-sm md:text-base"
                :class="activeTab === 'site_settings' ? 'bg-blue-600 text-white shadow-lg shadow-blue-900/50' : 'text-gray-400 hover:text-blue-400 hover:bg-blue-500/10'">
                        ...
                        <div class="p-6 space-y-8">
                            ...
                            <div class="space-y-3">
                                ...
                                <div class="relative w-full aspect-[2/1] bg-gray-900 rounded-2xl border-2 border-dashed border-gray-700 overflow-hidden group">
                                    ...
                                    <input type="file" name="site_cover_image" accept="image/*" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10" 
                                        onchange="document.getElementById('cover-preview').src = window.URL.createObjectURL(this.files[0])">
                                </div>
                                <p class="text-xs text-gray-500 mt-2">ภาพพื้นหลังที่จะแสดงทั่วทั้งเว็บไซต์ แนะนำขนาด: 1920x1080 px</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="space-y-8">
                        ...
                        <div class="bg-gray-800 rounded-2xl border border-gray-700 overflow-hidden shadow-xl relative">
                            <div class="px-6 py-4 bg-gray-900/80 border-b border-gray-700 flex justify-between items-center">
                                ...
                                <button type="button" @click="addReviewImage()" class="text-xs bg-amber-600/20 text-amber-400 px-2 py-1 rounded">เพิ่มรูปรีวิว</button>
                            </div>
                            <div class="p-6">
                                <div class="grid grid-cols-2 sm:grid-cols-3 gap-4">
                                    <template x-for="(review, index) in review_images" :key="index">
                                        <div class="relative group aspect-square bg-gray-900 rounded-xl border border-gray-700 overflow-hidden">
                                            <button type="button" @click="removeReviewImage(index)" class="absolute top-2 right-2 bg-red-600 text-white w-6 h-6 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 text-[10px] z-20"><i class="fas fa-times"></i></button>
                                            <input type="hidden" :name="`review_images[${index}][id]`" :value="review.id">
                                            <input type="hidden" :name="`review_images[${index}][existing_path]`" :value="review.existing_path">
                                            ...
                                            <input type="file" :name="`review_images[${index}][image]`" accept="image/*" @change="previewImage($event, 'review_images', index)" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10">
                                            
                                            <div class="absolute bottom-2 left-2 px-2 py-0.5 bg-black/60 rounded text-[10px] text-gray-400">
                                                ลำดับ: <span x-text="index+1"></span>
                                            </div>
                                        </div>
                                    </template>
                                </div>
                                <div x-show="review_images.length === 0" class="text-center py-8 text-gray-500 text-sm italic">
                                    ยังไม่มีรูปภาพรีวิว
                                </div>
                            </div>
                        </div>
