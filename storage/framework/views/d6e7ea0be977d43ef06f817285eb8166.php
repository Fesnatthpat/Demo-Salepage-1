...
<?php $__env->startSection('content'); ?>
            ...
            <div class="swiper-wrapper">
                ...
                <div class="swiper-slide">
                    <a href="/allproducts" class="block w-full">
                        ...
                        <img src="<?php echo e(asset('images/th-1.png')); ?>" class="w-full h-auto block"
                            alt="โปรโมชั่น Sale"
                ...
                <div class="swiper-slide">
                    <a href="/allproducts" class="block w-full">
                        <img src="<?php echo e(asset('images/th-2.png')); ?>" class="w-full h-auto block"
                            alt="จัดส่งวันไหน"
                ...
                <div class="swiper-slide">
                    <a href="/allproducts" class="block w-full">
                        <img src="<?php echo e(asset('images/th-3.png')); ?>" class="w-full h-auto block"
                            alt="ขอขอบคุณลูกค้า"
                            onerror="this.onerror=null;this.src='https://via.placeholder.com/1920x600/ef4444/ffffff?text=Image+Not+Found';" />
                    </a>
                </div>
                ...
                <div class="swiper-slide">
                    <a href="/allproducts" class="block w-full">
                        <img src="<?php echo e(asset('images/th-4.png')); ?>" class="w-full h-auto block"
                            alt="ข้อมูลฮาลาล"
                            onerror="this.onerror=null;this.src='https://via.placeholder.com/1920x600/ef4444/ffffff?text=Image+Not+Found';" />
                    </a>
                </div>
                ...
                <div class="swiper-slide">
                    <a href="/allproducts" class="block w-full">
                        <img src="<?php echo e(asset('images/th-5.png')); ?>" class="w-full h-auto block"
                            alt="โปรโมชั่นส่งฟรี"
                            onerror="this.onerror=null;this.src='https://via.placeholder.com/1920x600/ef4444/ffffff?text=Image+Not+Found';" />
                    </a>
                </div>
    ...
    <div class="w-full bg-red-50">
        <div class="container mx-auto">
            <img src="<?php echo e(asset('images/image_27e610.png')); ?>" 
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-8 text-center divide-x divide-gray-100">
                ...
                <?php $__currentLoopData = $serviceBarItems; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <div class="flex flex-col items-center gap-3 group cursor-default">
                        <div class="p-3 bg-red-50 rounded-full group-hover:bg-red-100 transition duration-300"><?php echo $item['icon']; ?></div>
                        <span class="text-base font-bold text-gray-700 group-hover:text-red-600 transition"><?php echo e($item['text']); ?></span>
                    </div>
                ...
                <?php if(isset($recommendedProducts) && count($recommendedProducts) > 0): ?>
                    <?php $__currentLoopData = $recommendedProducts; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $product): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        ...
                        <div class="card bg-white border border-gray-100 shadow-sm hover:shadow-xl hover:-translate-y-2 transition-all duration-300 group flex flex-col h-full rounded-2xl overflow-hidden">
                            <a href="<?php echo e(route('product.show', $product->pd_sp_id)); ?>" class="block overflow-hidden relative pt-[100%]">
                                <img src="<?php echo e($displayImage); ?>" alt="<?php echo e($product->pd_sp_name); ?>" class="absolute top-0 left-0 w-full h-full object-cover group-hover:scale-110 transition duration-700" onerror="this.onerror=null;this.src='https://via.placeholder.com/400x400.png?text=No+Image';" />
                                <?php if($isOnSale): ?>
                                    <div class="absolute top-3 right-3 bg-red-600 text-white px-3 py-1 rounded-full text-xs font-bold shadow-md animate-pulse">ลด <?php echo e(number_format($discountAmount)); ?>.-</div>
                                <?php endif; ?>
                            </a>
                            <div class="p-5 flex-1 flex flex-col">
                                <div class="mb-2">
                                    <h2 class="text-lg font-bold text-gray-800 leading-tight line-clamp-2 hover:text-red-600 transition cursor-pointer" onclick="window.location='<?php echo e(route('product.show', $product->pd_sp_id)); ?>'"><?php echo e($product->pd_sp_name); ?></h2>
                                ...
                                <div class="mt-auto">
                                    <div class="flex justify-between items-center mb-4">
                                        <div class="flex flex-col">
                                            <?php if($isOnSale): ?>
                                                <span class="text-xs text-gray-400 line-through">฿<?php echo e(number_format($originalPrice)); ?></span>
                                                <span class="text-xl font-black text-red-600">฿<?php echo e(number_format($finalSellingPrice)); ?></span>
                                            <?php else: ?>
                                                <span class="text-xl font-black text-red-600">฿<?php echo e(number_format($finalSellingPrice)); ?></span>
                                            <?php endif; ?>
                                        </div>
                                        <div class="text-xs font-semibold <?php echo e($product->pd_sp_stock > 0 ? 'text-green-600 bg-green-50' : 'text-red-500 bg-red-50'); ?> px-2 py-1 rounded-md"><?php echo e($product->pd_sp_stock > 0 ? 'มีสินค้า' : 'สินค้าหมด'); ?></div>
                                    </div>
                                    <button type="button" onclick="addToCartQuick(this, '<?php echo e(route('cart.add', ['id' => $product->pd_sp_id])); ?>')" class="btn w-full rounded-xl border-none font-bold text-white shadow-md transition-transform active:scale-95 <?php echo e($product->pd_sp_stock > 0 ? 'bg-red-600 hover:bg-red-700 shadow-red-200' : 'bg-gray-300 cursor-not-allowed'); ?>" <?php echo e($product->pd_sp_stock <= 0 ? 'disabled' : ''); ?>>
                                        <?php if($product->pd_sp_stock > 0): ?> ใส่ตะกร้าเลย <?php else: ?> สินค้าหมด <?php endif; ?>
                                    </button>
                                </div>
                            </div>
                        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            var swiper = new Swiper(".mySwiper", {
                ...
                loop: true,
                speed: 800,
                autoHeight: true, // ทำให้กล่องปรับความสูงอัตโนมัติ (แก้ปัญหารูปโดนตัดหรือพื้นที่ว่างเยอะเกิน)
                effect: 'slide',
                autoplay: {
                    delay: 4000, 
        ...
        function addToCartQuick(btnElement, url) {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    Swal.fire({
                        icon: 'success', title: 'เพิ่มเรียบร้อย!', showConfirmButton: false, timer: 1500,
                        position: 'top-end', toast: true, background: '#FEF2F2', iconColor: '#DC2626'
                    });
                    if (window.updateCartBadge) window.updateCartBadge(data.cartCount);
                } else {
                    Swal.fire({ icon: 'error', title: 'แจ้งเตือน', text: data.message || 'เพิ่มสินค้าไม่ได้', confirmButtonColor: '#DC2626' });
                }
            })
            .catch(err => Swal.fire({ icon: 'error', title: 'Error', text: 'Connection failed', confirmButtonColor: '#DC2626' }))
            .finally(() => {
                setTimeout(() => { btnElement.disabled = false; btnElement.innerHTML = originalHTML; }, 500);
            });
        }
    </script>
<?php $__env->stopSection(); ?>
